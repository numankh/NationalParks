...
public class FileUploadManager implements Serializable {
    ...
    public String handleFileUpload(FileUploadEvent event) throws IOException {
        ...
        String parkCode = getUserFileController().getParkCode();
        if(parkCode == null ||
                parkCode.length() != 4) {
            FacesMessage infoMessage = new FacesMessage(FacesMessage.SEVERITY_FATAL,
                    "No Park Selected!", "Please Select a Park!");
        ...
        try {
            ...
            Create a new UserFile object with attibutes: (See UserFile table definition inputStream DB)
                ...
                <> user_id = user
             */
            UserFile newUserFile = new UserFile(userId_filename, user, parkCode);
